...
/**
 * Proides an OpenGraph and plain HTML link fallback for embedding URLs.
 *
 * @since 2.3
 *
...
function gutenberg_oembed_fallback_response( $url ) {
	...
	// TODO: make the html safe by escaping the url, title, etc.
	...
	// simple link in a paragraph, editor will convert this to a paragraph block
	if ( ! $image && ! $desc ) {
		return array(
			'url'   => $url,
	...
	$html = "<blockquote><h1><a href=\"$url\">$title</a></h1>";
	if ( $desc ) {
		$html .= "<p>$desc</p>";
	}
	$html .= '</blockquote>';
 ...
 * Adds the Gutenberg embed fallback response, if oEmbed cannot embed the URL.
 *
 * @param  string  $html    HTML already generated by oEmbed.
 * @param  string  $url     The URL to embed.
